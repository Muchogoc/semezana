...
func (cq *ChannelQuery) QuerySubscriptions() *SubscriptionQuery {
	query := &SubscriptionQuery{config: cq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := cq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (cq *ChannelQuery) Clone() *ChannelQuery {
	...
	return &ChannelQuery{
		...
		withSubscriptions: cq.withSubscriptions.Clone(),
...
func (cq *ChannelQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Channel, error) {
	...
	if query := cq.withMembers; query != nil {
		if err := cq.loadMembers(ctx, query, nodes,
			func(n *Channel) { n.Edges.Members = []*User{} },
			func(n *Channel, e *User) { n.Edges.Members = append(n.Edges.Members, e) }); err != nil {
			return nil, err
		}
	}
	if query := cq.withSubscriptions; query != nil {
		if err := cq.loadSubscriptions(ctx, query, nodes,
			func(n *Channel) { n.Edges.Subscriptions = []*Subscription{} },
			func(n *Channel, e *Subscription) { n.Edges.Subscriptions = append(n.Edges.Subscriptions, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (cq *ChannelQuery) loadMessages(ctx context.Context, query *MessageQuery, nodes []*Channel, init func(*Channel), assign func(*Channel, *Message)) error {
	...
	query.Where(predicate.Message(func(s *sql.Selector) {
		s.Where(sql.InValues(channel.MessagesColumn, fks...))
	}))
...
func (cq *ChannelQuery) loadMembers(ctx context.Context, query *UserQuery, nodes []*Channel, init func(*Channel), assign func(*Channel, *User)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[uuid.UUID]*Channel)
	nids := make(map[uuid.UUID]map[*Channel]struct{})
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(channel.MembersTable)
		s.Join(joinT).On(s.C(user.FieldID), joinT.C(channel.MembersPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(channel.MembersPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(channel.MembersPrimaryKey[1]))
	...
	neighbors, err := query.sqlAll(ctx, func(_ context.Context, spec *sqlgraph.QuerySpec) {
		...
		spec.Assign = func(columns []string, values []interface{}) error {
			...
			if nids[inValue] == nil {
				nids[inValue] = map[*Channel]struct{}{byID[outValue]: struct{}{}}
...
func (cq *ChannelQuery) loadSubscriptions(ctx context.Context, query *SubscriptionQuery, nodes []*Channel, init func(*Channel), assign func(*Channel, *Subscription)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*Channel)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.Subscription(func(s *sql.Selector) {
		s.Where(sql.InValues(channel.SubscriptionsColumn, fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
